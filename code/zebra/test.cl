...
__kernel void kernel_test( global const unsigned char *img_original, global unsigned char *img, int array_size, int width, global const int *mask, int mask_size )
{
    const int2 pos = {get_group_id(0)*get_local_size(0) + get_local_id(0), get_group_id(1)*get_local_size(1) + get_local_id(1)};
    ...
    img[ 3 * (pos.x * width + pos.y) + 0 ] = (unsigned char)(sum_r_x / 4096 / 4096);
    img[ 3 * (pos.x * width + pos.y) + 1 ] = (unsigned char)(sum_g_x / 4096 / 4096);
    img[ 3 * (pos.x * width + pos.y) + 2 ] = (unsigned char)(sum_b_x / 4096 / 4096);
}